        ...
        <View className="mt-5 self-center">
          ...
          <View className="flex flex-row items-center gap-3 mt-5 justify-between">
            {ContinueWithData.map((item, index) => (
              <TouchableOpacity key={index} onPress={handleSignInWithProvider} className='rounded-full p-2'>
                <Image
                  source={item.image}
                  className=" w-10 h-10"
                  resizeMode="contain"
          ...
          <View className='flex-row my-8'>
            <Text className='text-gray-700 text-lg self-center'> Create An Account </Text>
            <TouchableOpacity onPress={() => props.navigation.navigate('Signup')}>
              <Text className='text-red-500 text-lg font-bold underline self-center'> Sign Up </Text>
            </TouchableOpacity>
          </View>
        </View>
      </View>
